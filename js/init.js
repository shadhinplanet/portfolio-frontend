...
function myfolio__color_switcher(){
	...
	list.on('click',function(){
		...
		element.closest('.myfolio__all_wrap').attr('data-color',''+elval+'');
		return false;
	});
}
...
function myfolio__nav_bg(){
	...
	jQuery(window).on('scroll',function(){
		...
		if(WinOffset >= 100){
			...
			list.each(function(i){
				var ele = jQuery(this);
				setTimeout(function(){ele.removeClass('opened');},i*50);
			});
		}
	});
}
...
function myfolio__trigger_menu(){
	...
	var mobileMenuList	= jQuery('.myfolio__mobile_menu .dropdown .dropdown_inner ul li a');
...
function myfolio__my_load(){
	...
	var speed	= 500;
	setTimeout(function(){myfolio__preloader();},speed);
...
function myfolio__cursor(){
	...
	if(myCursor.length){
        ...
        }, $("body").on("mouseenter", "a, .myfolio__topbar .trigger, .cursor-pointer", function () {
            e.classList.add("cursor-hover"), t.classList.add("cursor-hover")
        }), $("body").on("mouseleave", "a, .myfolio__topbar .trigger, .cursor-pointer", function () {
